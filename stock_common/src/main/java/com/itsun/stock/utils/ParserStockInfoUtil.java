...
public class ParserStockInfoUtil {
    ...
    private StockRtInfo parser4StockRtInfo(String stockCode, String otherInfo) {
        ...
        stockRtInfo.setStockCode(stockCode);
        stockRtInfo.setStockName(stockName);
        stockRtInfo.setOpenPrice(openPrice);
        stockRtInfo.setPreClosePrice(preClosePrice);
        stockRtInfo.setCurPrice(currentPrice);
        stockRtInfo.setMaxPrice(maxPrice);
        stockRtInfo.setMinPrice(minPrice);
        stockRtInfo.setTradeAmount(tradeAmount);
        stockRtInfo.setTradeVolume(tradeVol);
        stockRtInfo.setCurTime(curDateTime);
        return stockRtInfo;
    }
    ...
    private StockMarketIndexInfo parser4InnerStockMarket(String marketCode, String otherInfo) {
        ...
        //组装实体对象
        StockMarketIndexInfo smi = new StockMarketIndexInfo();
        smi.setId(idWorker.nextId());
        smi.setMarketCode(marketCode);
        smi.setMarketName(marketName);
        smi.setCurPoint(curPoint);
        smi.setOpenPoint(openPoint);
        smi.setPreClosePoint(preClosePoint);
        smi.setMaxPoint(maxPoint);
        smi.setMinPoint(minPoint);
        smi.setTradeVolume(tradeVol);
        smi.setTradeAmount(tradeAmt);
        smi.setCurTime(curTime);
        return smi;
    }
    ...
    public List<StockBlockRtInfo> parse4StockBlock(String stockStr){
        ...
        List<StockBlockRtInfo> collect = (List<StockBlockRtInfo>) mapInfo.values().stream().map(restStr -> {
            ...
            Date now=DateTimeUtil.getDateTimeWithoutSecond(DateTime.now()).toDate();
            //构建板块信息对象
            StockBlockRtInfo blockRtInfo =new StockBlockRtInfo();
            blockRtInfo.setId(idWorker.nextId());
            blockRtInfo.setLabel(label);
            blockRtInfo.setBlockName(blockName);
            blockRtInfo.setCompanyNum(companyNum);
            blockRtInfo.setAvgPrice(avgPrice);
            blockRtInfo.setCurTime(now);
            blockRtInfo.setUpdownRate(priceLimit);
            blockRtInfo.setTradeAmount(tradeAmount);
            blockRtInfo.setTradeVolume(tradeVolume);
            return blockRtInfo;
